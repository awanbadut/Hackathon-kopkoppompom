...
interface RewardsTabProps {
  ...
  handleClaimMission: (code: string, points: number) => void;
  vouchers: any[];
  userPoints: any;
      ...
      <div className="bg-white border border-stone-200 p-6 rounded-3xl shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-200 pb-3">
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            <div className="bg-stone-50 border border-stone-200 px-4 py-2 rounded-2xl flex items-center gap-2">
              ...
              <span className="text-xs font-black text-stone-800 font-mono">
                Estimasi SHU: {totalKoperasiPoints > 0 ? ((pointsBalance / totalKoperasiPoints) * 100).toFixed(2) : '0.00'}%
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
            ...
            <div key={misi.code} className="p-4 border border-stone-205 rounded-2xl flex flex-col justify-between gap-4 bg-stone-50/50">
              <div className="space-y-2">
                ...
                <h4 className="text-xs font-black text-stone-800">{misi.title}</h4>
                <p className="text-[10px] text-stone-550 leading-relaxed font-medium">{misi.desc}</p>
              </div>
              ...
              <div className="pt-3 border-t border-dashed border-stone-200 flex items-center justify-between">
                <span className="text-[10px] font-bold">
                  {misi.claimed ? (
                    <span className="text-[#548C2F] flex items-center gap-1">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Poin Diklaim
                    </span>
                  ) : misi.completed ? (
                    <span className="text-[#E08E10] font-black animate-pulse">Selesai • Siap Klaim</span>
                  ) : (
                ...
                {misi.claimed ? (
                  <button disabled className="py-1 px-3 bg-stone-100 text-stone-400 text-[10px] font-bold rounded-lg cursor-not-allowed">
                    Claimed
                  </button>
                ) : misi.completed ? (
                  <button
                  ...
                  <button disabled className="py-1 px-3 bg-stone-100 text-stone-400 text-[10px] font-bold rounded-lg cursor-not-allowed border border-stone-200">
                    Belum Selesai
                  </button>
                )}
              </div>
        ...
        <div className="md:col-span-2 bg-white border border-stone-200 p-6 rounded-3xl shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {vouchers.map((voucher) => {
              ...
              return (
                <div key={voucher.id} className="p-4 border border-stone-200 rounded-2xl flex flex-col justify-between gap-4 bg-stone-50">
                  <div>
                    ...
                    <h4 className="mt-3 text-xs font-black text-stone-800 leading-snug">{voucher.title}</h4>
                    <p className="text-[10px] text-stone-550 mt-1 leading-relaxed">{voucher.description}</p>
                  </div>
        ...
        <div className="md:col-span-1 bg-white border border-stone-200 p-6 rounded-3xl shadow-sm space-y-4">
          ...
          <div className="space-y-3.5">
            {myRedeemedVouchers.map((uv) => (
              <div key={uv.id} className="p-3 border border-dashed border-[#F9A620]/50 rounded-2xl bg-white shadow-sm space-y-3">
