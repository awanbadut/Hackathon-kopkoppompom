...
async function createApprovalRequests(txId) {
    ...
    const { rows: managers } = await db_1.db.query(`SELECT id, role, full_name FROM ${(0, db_1.p)('app_users')} 
     WHERE koperasi_ref = $1 AND status = 'active' AND id <> $2`, [koperasi_ref, input_by]);
    ...
    await db_1.db.query(`DELETE FROM ${(0, db_1.p)('approvals')} WHERE transaction_id = $1`, [txId]);
    ...
    for (const req of requiredApprovals) {
        await db_1.db.query(`INSERT INTO ${(0, db_1.p)('approvals')} (transaction_id, approver_id, approval_level, status) 
       VALUES ($1, $2, $3, 'menunggu')`, [txId, req.approverId, req.level]);
    }
    // Update transaction status to 'menunggu_approval'
    await db_1.db.query(`UPDATE ${(0, db_1.p)('transaksi_keuangan')} SET status = 'menunggu_approval' WHERE id = $1`, [txId]);
...
async function decideApproval(approvalId, deciderId, status, catatan) {
    // Fetch approval details with transaction
    const { rows } = await db_1.db.query(`SELECT a.*, t.input_by FROM ${(0, db_1.p)('approvals')} a 
     JOIN ${(0, db_1.p)('transaksi_keuangan')} t ON a.transaction_id = t.id 
    ...
    await db_1.db.query(`UPDATE ${(0, db_1.p)('approvals')} SET status = $1, catatan = $2, decided_at = $3 WHERE id = $4`, [status, catatan || null, new Date().toISOString(), approvalId]);
    ...
    if (status === 'ditolak') {
        ...
        await db_1.db.query(`UPDATE ${(0, db_1.p)('transaksi_keuangan')} SET status = 'ditolak' WHERE id = $1`, [txId]);
        // Cancel all other pending approvals for this transaction
        await db_1.db.query(`UPDATE ${(0, db_1.p)('approvals')} SET status = 'ditolak', catatan = 'Ditolak oleh approver lain' 
       WHERE transaction_id = $1 AND status = 'menunggu'`, [txId]);
        // Notify submitter
        await (0, notifications_1.sendNotification)({
            user_id: inputBy,
        ...
        const { rows: allApps } = await db_1.db.query(`SELECT status FROM ${(0, db_1.p)('approvals')} WHERE transaction_id = $1`, [txId]);
        const allApproved = allApps.every(a => a.status === 'disetujui');
        if (allApproved) {
            // Update transaction status to approved
            await db_1.db.query(`UPDATE ${(0, db_1.p)('transaksi_keuangan')} SET status = 'disetujui' WHERE id = $1`, [txId]);
            // Notify submitter
            await (0, notifications_1.sendNotification)({
                user_id: inputBy,
                type: 'transaction_status',
