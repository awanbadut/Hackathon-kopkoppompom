...
export interface RiskLogInsert {
  transaction_id: string;
  rule_code: string;
  risk_level: 'perlu_perhatian' | 'berisiko_tinggi';
  message: string;
}

export async function evaluateTransactionRisk(txId: string): Promise<{
  triggeredRules: RiskLogInsert[];
  finalRiskLevel: 'aman' | 'perlu_perhatian' | 'berisiko_tinggi';
}> {
  ...
  const triggeredRules: RiskLogInsert[] = [];
  ...
  if (tx.sumber_dana === 'dana_desa') {
    const { rows: kwRows } = await db.query(
      `SELECT kode_wilayah FROM ${p('referensi_koperasi_wilayah')} WHERE koperasi_ref = $1`,
      [tx.koperasi_ref]
    );
    ...
    if (kopWil?.kode_wilayah) {
      const { rows: pdRows } = await db.query(
        `SELECT anggaran_dana_desa FROM ${p('referensi_profil_desa')} WHERE kode_wilayah = $1`,
        [kopWil.kode_wilayah]
      );
      ...
      if (profDesa?.anggaran_dana_desa) {
        ...
        const { rows: yearRows } = await db.query(
          `SELECT SUM(amount) as total FROM ${p('transaksi_keuangan')} 
           WHERE koperasi_ref = $1 AND sumber_dana = 'dana_desa' AND status = 'disetujui' 
           AND transaction_date >= $2`,
          [tx.koperasi_ref, `${currentYear}-01-01`]
        );
  ...
  if (isOutflow) {
    const { rows: sumRows } = await db.query(
      `SELECT saldo_kas FROM ${p('v_koperasi_summary')} WHERE koperasi_ref = $1`,
      [tx.koperasi_ref]
    );
  ...
  const { rows: dupRows } = await db.query(
    `SELECT id FROM ${p('transaksi_keuangan')} 
     WHERE koperasi_ref = $1 AND amount = $2 AND description = $3 
     AND transaction_date = $4 AND id <> $5 LIMIT 1`,
    [tx.koperasi_ref, tx.amount, tx.description, tx.transaction_date, txId]
  );
  ...
  if (Number(tx.amount) > 10000000) {
    const { rows: appCount } = await db.query(
      `SELECT COUNT(*)::int as count FROM ${p('approvals')} WHERE transaction_id = $1 AND status = 'disetujui'`,
      [txId]
    );
  ...
  const { rows: ratRows } = await db.query(
    `SELECT MAX(tanggal_rat) as last_rat FROM ${p('rat_koperasi')} WHERE koperasi_ref = $1`,
    [tx.koperasi_ref]
  );
  ...
  if (tx.type === 'pinjaman') {
    ...
    const { rows: allPinjaman } = await db.query(
      `SELECT amount, kategori FROM ${p('transaksi_keuangan')} 
  ...
  let finalRiskLevel: 'aman' | 'perlu_perhatian' | 'berisiko_tinggi' = 'aman';
  ...
  // Clear previous logs
  await db.query(
    `DELETE FROM ${p('risk_logs')} WHERE transaction_id = $1`,
    [txId]
  );
  ...
  if (triggeredRules.length > 0) {
    ...
    for (const rule of triggeredRules) {
      await db.query(
        `INSERT INTO ${p('risk_logs')} (transaction_id, rule_code, risk_level, message) 
         VALUES ($1, $2, $3, $4)`,
        [txId, rule.rule_code, rule.risk_level, rule.message]
      );
    }
  }
  ...
  await db.query(
    `UPDATE ${p('transaksi_keuangan')} SET risk_level = $1 WHERE id = $2`,
    [finalRiskLevel, txId]
  );
...
export async function getDetailedHealthMetrics(koperasiRef: string): Promise<{
  kesehatan_kas: number;
  kepatuhan: number;
  kesehatan_koperasi: number;
  partisipasi_anggota: number;
}> {
  // 1. Calculate Kepatuhan (Compliance Score)
  const { rows: riskRows } = await db.query(
    `SELECT r.risk_level, r.rule_code, r.resolved 
     FROM ${p('risk_logs')} r 
  ...
  for (const r of riskRows) {
    ...
    if (r.rule_code === 'R03_OVER_BUDGET') hasOverBudget = true;
    if (r.rule_code === 'R01_NO_EVIDENCE') noEvidenceCount++;
    if (r.rule_code === 'R07_ROUND_NUMBER_PATTERN') roundNumberCount++;
  }
  ...
  if (hasOverBudget) kesehatan_kas -= 40;
  ...
  const { rows: memberStats } = await db.query(
    `SELECT 
  ...
  const kesehatan_koperasi = Math.round(
    (0.4 * kesehatan_kas) + (0.4 * kepatuhan) + (0.2 * partisipasi_anggota)
  );
...
export async function calculateKoperasiHealthScore(koperasiRef: string): Promise<number> {
  const metrics = await getDetailedHealthMetrics(koperasiRef);
  
  await db.query(
    `INSERT INTO ${p('koperasi_health_score')} (koperasi_ref, score) VALUES ($1, $2)`,
    [koperasiRef, metrics.kesehatan_koperasi]
  );
